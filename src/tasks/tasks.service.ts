...
export class TasksService {
     ...
     */
    async saveTask(workspaceId: bigint, creatorId: bigint, workspaceTimezone: string, data: any) {
        const { id, date, time, description, contact_id, user_id } = data;

        if (!date || !time || !description || !contact_id) {
            throw new BadRequestException('Date, Time, Description and Contact are required');
        }
        ...
        if (id) {
            ...
            this.logger.log(`Task created: linking to inbox for contact ${contact_id}`);
        }

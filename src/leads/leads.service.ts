...
export class LeadsService {
    ...
    async getLeads(workspaceId: bigint, filters: any) {
        ...
        const where: any = { workspace_id: workspaceId, type: 'lead', deleted_at: null };
        ...
        const orderBy = filters.sort_by ? { [filters.sort_by]: filters.sort_order || 'desc' } : { created_at: 'desc' };
        ...
        const [leads, total] = await Promise.all([
            this.prisma.contacts.findMany({
                ...
                skip: (page - 1) * limit,
                take: limit,
                include: { custom_field_entities: { include: { custom_field_entity_values: true } }, tags: true }
            }),
        ...
        return { leads: mappedLeads, total, page, limit, last_page: Math.ceil(total / limit) };
    }
    ...
    prepareLeadData(lead: any) {
        ...
        const profile_completion = totalFields > 0 ? Math.round((filledFields / totalFields) * 100) : 0;
    ...
    async addLead(data: any, workspaceId: bigint, userId: bigint) {
        const leadData: any = {
            workspace_id: workspaceId, creator_id: userId, type: 'lead',
            first_name: data.first_name || '', last_name: data.last_name || '',
            email: data.email || null, phone: data.phone || null,
            source: data.source || 'manual', status: data.status || 'new',
        };
        ...
        if (data.custom_fields && typeof data.custom_fields === 'object') {
            const entity = await this.prisma.custom_field_entities.create({
                data: { entity_id: lead.id, entity_type: 'contact', workspace_id: workspaceId }
            });
        ...
        return { success: true, lead: this.prepareLeadData({ ...lead, custom_field_entities: [] }) };
    }

    async updateLead(data: any, workspaceId: bigint) {
        if (!data.id) throw new BadRequestException('Lead ID required');
        const lead = await this.prisma.contacts.findFirst({ where: { id: BigInt(data.id), workspace_id: workspaceId } });
        if (!lead) throw new NotFoundException('Lead not found');

        const updateData: any = {};
        ['first_name', 'last_name', 'email', 'phone', 'status', 'source'].forEach(key => {
    ...
    async trashLead(leadId: bigint, workspaceId: bigint) {
        const lead = await this.prisma.contacts.findFirst({ where: { id: leadId, workspace_id: workspaceId } });
        if (!lead) throw new NotFoundException('Lead not found');
        await this.prisma.contacts.update({ where: { id: leadId }, data: { deleted_at: new Date() } });
    ...
    async updateCustomData(slug: string, data: any, workspaceId: bigint) {
        ...
        if (!entity) {
            entity = await this.prisma.custom_field_entities.create({
                data: { entity_id: BigInt(data.contact_id), entity_type: 'contact', workspace_id: workspaceId }
            });
        }
        ...
        if (existingValue) {
            ...
            await this.prisma.custom_field_entity_values.create({
                data: { entity_id: entity.id, slug, value: data.value === null ? '' : String(data.value) }
            });
        }
    ...
    async deleteCustomField(valueId: bigint) {
        await this.prisma.custom_field_entity_values.deleteMany({ where: { id: valueId } });

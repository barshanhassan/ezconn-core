...
export class AutomationsService {
    ...
    async createAutomation(workspaceId: bigint, data: any) {
        ...
        return { success: true, automation: { ...automation, draft_version_id: version.id } };
    }
    ...
    async publishAutomation(workspaceId: bigint, automationId: bigint) {
        ...
        const liveVersion = await this.prisma.automation_versions.create({
            data: {
                automation_id: automation.id,
                number: (await this.prisma.automation_versions.count({ where: { automation_id: automation.id } })) + 1,
                status: 'published',

...
export class BundlesService {
    ...
    async createBundle(data: any, workspaceId: number, userId: bigint) {
        ...
        const slug = data.slug || `${data.name.toLowerCase().replace(/ /g, '-')}-${uuidv4().split('-')[0]}`;
    ...
    async updateBundle(bundleId: bigint, data: any, workspaceId: number) {
        const bundle = await this.prisma.bundles.findFirst({ where: { id: bundleId, workspace_id: workspaceId } });
        ...
        const updated = await this.prisma.bundles.update({
            ...
            data: {
                ...
                published_at: data.published && !bundle.published ? new Date() : bundle.published_at,
            }
        });
    ...
    async bulkAddResources(bundleId: bigint, resources: any[]) {
        ...
        await this.prisma.bundle_resources.createMany({ data, skipDuplicates: true });
        return { success: true };
    }
    ...
    async import(shareId: bigint, workspaceId: number) {
        const share = await this.prisma.bundle_shares.findUnique({ where: { id: shareId } });
        ...
        console.log(`Stub: Importing bundle ${bundle.name} into workspace ${workspaceId}`);

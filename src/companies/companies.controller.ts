...
export class CompaniesController {
    ...
    async assignCompany(@Body() body: any, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        return this.service.assignCompany(workspaceId, BigInt(body.contact_id), BigInt(body.company_id));
    }

    @Post(':id/status')
    async changeStatus(@Param('id') id: string, @Body('action') action: string, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);

...
export class ReportsService {
    ...
    async store(data: any, workspaceId: bigint) {
        if (!data.name || !data.type || !data.model || !data.prompt) {
            throw new BadRequestException('name, type, model, and prompt are required');
        }
    ...
    async update(reportId: bigint, data: any, workspaceId: bigint) {
        const report = await this.prisma.reports.findFirst({ where: { id: reportId, workspace_id: workspaceId } });
        ...
        const updated = await this.prisma.reports.update({ where: { id: reportId }, data: updateData });
    ...
    async run(reportId: bigint, data: any, workspaceId: bigint) {
        const report = await this.prisma.reports.findFirst({ where: { id: reportId, workspace_id: workspaceId } });
        if (!report) throw new NotFoundException('Report not found');
        await this.prisma.reports.update({ where: { id: reportId }, data: { run_started_at: new Date() } });
        // Stub: Dispatch report generation job
        console.log(`Stub: Generating report ${reportId} with contact ${data.contact}`);
        return { report };
    }

...
export class TelegramService {
    ...
    async addBot(data: any, workspaceId: bigint, userId: bigint) {
        if (!data.name || !data.token) throw new BadRequestException('Name and token are required');

        // Check duplicate token
        const exists = await this.prisma.telegram_bots.count({ where: { token: data.token } });
        if (exists > 0) throw new BadRequestException('This token is already in use');

        const bot = await this.prisma.telegram_bots.create({ data: {
    ...
    async updateBot(botId: bigint, data: any, workspaceId: bigint, userId: bigint) {
        ...
        if (data.newToken) {
            const tokenExists = await this.prisma.telegram_bots.count({ where: { token: data.newToken } });
            if (tokenExists > 0) throw new BadRequestException('This token is already in use');
            updateData.old_token = bot.token;
            updateData.token = data.newToken;
            updateData.status = 'PENDING';
        }

        const updated = await this.prisma.telegram_bots.update({ where: { id: botId }, data: updateData });
        return { bot: updated };
    }
    ...
    async reconnectBot(botId: bigint, workspaceId: bigint) {
        ...
        await this.prisma.telegram_bots.update({
            where: { id: botId }, data: { status: 'ACTIVE' }
        });
        const updated = await this.prisma.telegram_bots.findFirst({ where: { id: botId } });
        return { bot: updated };
    }

    async deleteBot(botId: bigint, workspaceId: bigint, userId: bigint, deleteFolder?: boolean) {
        const bot = await this.prisma.telegram_bots.findFirst({
            where: { id: botId, workspace_id: workspaceId }
        });
        if (!bot) throw new BadRequestException('Bot not found');
        ...
        return { message: 'Deletion is currently in process. You will be notified when it is deleted.' };
    }

    async updateBotUsers(botId: bigint, userIds: bigint[], workspaceId: bigint, userId: bigint) {
        const bot = await this.prisma.telegram_bots.findFirst({
            where: { id: botId, workspace_id: workspaceId }
        });
        if (!bot) throw new BadRequestException('Bot not found');
        ...
        for (const uid of userIds) {
            ...
            if (existing) {
                ...
                const created = await this.prisma.telegram_bot_users.create({ data: { telegram_bot_id: botId, user_id: uid }
                });
    ...
    async messageAction(messageId: bigint, action: string) {
        ...
        const message = await this.prisma.telegram_messages.findFirst({ where: { id: messageId } });
    ...
    async updateAutoReply(botId: bigint, data: any, workspaceId: bigint) {
        ...
        if (data.auto_reply_automation_id !== undefined) {
            updateData.auto_reply_automation_id = data.auto_reply_automation_id ? BigInt(data.auto_reply_automation_id) : null;
    ...
    async createAutoReply(data: any, workspaceId: bigint, userId: bigint) {
        if (!data.telegram_bot_id || !data.slug) throw new BadRequestException('telegram_bot_id and slug required');
        ...
        const automation = await this.prisma.automations.create({ data: {
                workspace_id: workspaceId,
                creator_id: userId,
        ...
        const version = await this.prisma.automation_versions.create({ data: { automation_id: automation.id, number: BigInt(1), status: 'draft' }
        });
        ...
        const triggerStep = await this.prisma.automation_steps.create({ data: {
        ...
        await this.prisma.automation_step_activities.create({ data: {
    ...
    async toggleFeeder(botId: bigint) {
        const bot = await this.prisma.telegram_bots.findFirst({ where: { id: botId } });
        if (!bot) throw new NotFoundException('Bot not found');

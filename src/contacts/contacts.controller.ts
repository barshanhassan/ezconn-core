...
export class ContactsController {
    ...
    @Patch(':id')
    async updateContactData(@Param('id') id: string, @Body() body: any, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        return this.service.updateContactData(workspaceId, BigInt(id), body);
    }

    @Post(':id/pause-automations')
    async pauseAutomations(@Param('id') id: string, @Body('minutes') minutes: number, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        return this.service.pauseAutomations(workspaceId, BigInt(id), minutes || 60);
    }

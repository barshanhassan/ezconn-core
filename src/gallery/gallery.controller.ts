import { Controller, Get, Post, Patch, Delete, Body, Query, UseGuards, Request, Param, UseInterceptors, UploadedFiles } from '@nestjs/common';
...
export class GalleryController {
    ...
    async uploadFile(@UploadedFiles() files: Express.Multer.File[], @Body() body: any, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        const userId = BigInt(req.user.sub || 1);
        return this.service.uploadFiles(files, body.parent_id, workspaceId, userId);
    }
    ...
    async createFolder(@Body() body: any, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        const userId = BigInt(req.user.sub || 1);
        return this.service.createFolder(body.name, workspaceId, userId);
    }
    ...
    @Patch('rename/:id')
    async renameObject(@Param('id') id: string, @Body() body: any, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        const userId = BigInt(req.user.sub || 1);

...
export class BroadcastsService {
    ...
    async createBroadcast(workspaceId: bigint, creatorId: bigint, data: any) {
        ...
        const broadcast = await this.prisma.broadcasts.create({
            data: {
                ...
                wa_template_id: data.wa_template_id ? parseInt(data.wa_template_id) : null,
    ...
    async updateBroadcast(broadcastId: bigint, workspaceId: bigint, data: any) {
        ...
        const updated = await this.prisma.broadcasts.update({
            ...
            data: {
                ...
                wa_template_id: data.wa_template_id ? parseInt(data.wa_template_id) : undefined,
    ...
    async getBroadcastAudience(broadcastId: bigint, workspaceId: bigint, query: any) {
        const broadcast = await this.prisma.broadcasts.findFirst({
            where: { id: broadcastId, workspace_id: workspaceId }
        });
        ...
        this.logger.debug(`Fetching audience for broadcast ${broadcastId} with filters: ${JSON.stringify(filters)}`);
    ...
    async exportBroadcastAudience(broadcastId: bigint, workspaceId: bigint) {
        ...
        return { success: true, message: 'Export initiated. You will be notified when the CSV is ready.' };
    }
}

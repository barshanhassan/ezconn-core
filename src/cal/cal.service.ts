    ...
    try {
      if (account.version === 'v2') {
        const response = await fetch(
          `${this.calApiUrl}v2/event-types?username=${account.username}`,
          {
            headers: {
        ...
        const response = await fetch(
          `${this.calApiUrl}v1/event-types?apiKey=${account.api_key}`,
        );
        const data = await response.json();
  ...
  async getSlots(workspaceId: bigint, params: any) {
    ...
    try {
      if (account.version === 'v2') {
        ...
        const end = new Date(
          new Date(booking_date).getTime() + 86400000,
        ).toISOString();
        ...
        const response = await fetch(
          `${this.calApiUrl}v2/slots?${queryParams}`,
          {
            headers: {
              Authorization: `Bearer ${account.api_key}`,
        ...
        const response = await fetch(
          `${this.calApiUrl}v1/slots?${queryParams}`,
        );
        const data = await response.json();
        return data.slots || [];
      }

...
export class CalController {
    ...
    @Get('event-types')
    async getEventTypes(@Query('account_id') accountId: string, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        return this.service.getEventTypes(workspaceId, accountId ? BigInt(accountId) : undefined);
    }

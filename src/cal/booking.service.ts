...
export class BookingService {
    ...
    async createBooking(workspaceId: bigint, userId: bigint, data: any) {
        const { event_type_id, start, timezone, responses, contact_id, account_id } = data;
        const account = await this.calService.getAccount(workspaceId, account_id ? BigInt(account_id) : undefined);

        if (!account || !account.api_key) {
            throw new BadRequestException('Cal.com account not found or API key missing');
        }
        ...
        try {
            if (account.version === 'v2') {
                ...
                calResponse = calResponse.status === 'success' ? calResponse.data : null;
            } else {
                const payload = {
                    eventTypeId: parseInt(event_type_id),
                    start: new Date(start).toISOString(),
                ...
                const res = await fetch(`${this.calApiUrl}v1/bookings?apiKey=${account.api_key}`, {
                    method: 'POST',
        ...
        if (!calResponse) throw new BadRequestException('Booking API returned empty response');
        ...
        if (account && account.api_key) {
            try {
                if (account.version === 'v2') {
                    ...
                    await fetch(`${this.calApiUrl}v1/bookings/${booking.uid}/cancel?apiKey=${account.api_key}`, {
                        method: 'DELETE'
                    });
                }
            } catch (e) {
                this.logger.warn(`Failed to cancel booking on Cal.com side: ${e.message}`);
            }
        }

import { Controller, Post, Get, Delete, Body, UseGuards, Request, UseInterceptors, UploadedFile } from '@nestjs/common';
...
export class UsersController {
    ...
    @Post('language')
    async updateLanguage(@Body('language') language: string, @Request() req: any) {
        const userId = BigInt(req.user.sub || 1);
    ...
    @Post('availability')
    async setAvailability(@Body('is_online') isOnline: number, @Request() req: any) {
        const userId = BigInt(req.user.sub || 1);
    ...
    async uploadProfileLogo(@UploadedFile() file: Express.Multer.File, @Body('logo') base64Logo: string, @Request() req: any) {
        const userId = BigInt(req.user.sub || 1);
        // Handle both standard file upload OR base64 payload
        return this.service.uploadProfileLogo(userId, file || base64Logo, !!base64Logo);
    }

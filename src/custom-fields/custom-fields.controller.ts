...
export class CustomFieldsController {
    ...
    async checkNameAvailability(@Query('system_name') systemName: string, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
    ...
    async changeFolder(@Body() body: any, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        return this.service.changeFolder(workspaceId, BigInt(body.tag_id), body.folder_id ? BigInt(body.folder_id) : null);
    }

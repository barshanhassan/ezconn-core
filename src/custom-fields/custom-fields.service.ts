...
export class CustomFieldsService {
    ...
    async createField(workspaceId: bigint, userId: bigint, data: any) {
        const { label, content_type, input_type, slug, system_name, properties } = data;
        ...
        if (slug) {
            ...
            // Create
            if (!system_name) throw new BadRequestException('System name is required');
    ...
    async removeProperty(workspaceId: bigint, propertyName: string) {
        ...
        if (!property || property.custom_fields.workspace_id !== workspaceId) {
    ...
    async changeFolder(workspaceId: bigint, fieldId: bigint, folderId: bigint | null) {
        const field = await this.prisma.custom_fields.findFirst({
            where: { id: fieldId, workspace_id: workspaceId }
        });
        if (!field) throw new NotFoundException('Field not found');

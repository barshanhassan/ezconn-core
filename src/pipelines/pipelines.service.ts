...
export class PipelinesService {
    ...
    async createPipeline(workspaceId: bigint, userId: bigint, data: any) {
        ...
        const defaultSteps = ['New', 'Contacted', 'Proposal', 'Negotiation', 'Closed'];
        await Promise.all(defaultSteps.map((stepName, index) => {
        ...
        return { success: true, pipeline: await this.prisma.pipelines.findUnique({ where: { id: pipeline.id }, include: { pipeline_steps: true } }) };
    }
    ...
    async createOpportunity(workspaceId: bigint, userId: bigint, data: any) {
        const { title, pl_id, pl_step_id, contact_id, company_id, value, closing_date } = data;
     ...
     */
    async updateOpportunityStatus(workspaceId: bigint, opportunityId: bigint, status: string) {
    ...
    async getStepStats(workspaceId: bigint, stepId: bigint) {
        const opportunities = await this.prisma.pipeline_opportunities.findMany({
            where: { pl_step_id: stepId, workspace_id: workspaceId, status: 'ACTIVE' }
        });

        const totalDeals = opportunities.length;
        const totalValue = opportunities.reduce((acc, opt) => acc + (opt.value || 0), 0);
        const expectedValue = opportunities.reduce((acc, opt) => acc + (opt.value * (opt.probability || 5) / 100), 0);
     ...
     */
    async sortOpportunities(workspaceId: bigint, stepId: bigint, sortedIds: string[]) {

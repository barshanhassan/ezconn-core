...
export class AiFeedersService {
  ...
  async addFeeder(workspaceId: bigint, creatorId: bigint, data: any) {
    ...
    if (!name || !ai_agent_id)
      throw new BadRequestException('Name and Agent ID are required');
  ...
  async publishTopic(workspaceId: bigint, topicId: bigint) {
    ...
    const content = topic.ai_questions
      .map((q) => `Q: ${q.question}\nA: ${q.answer}`)
      .join('\n\n');

    // This content would then be synced with OpenAI as a file
    this.logger.debug(
      `Publishing topic ${topicId} with ${topic.ai_questions.length} questions`,
    );

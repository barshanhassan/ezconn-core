...
export class AiFeedersController {
    ...
    @Post(':id/questions')
    async addAQuestion(@Param('id') id: string, @Body() body: any, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        const creatorId = BigInt(req.user.sub || 1);
    ...
    @Patch('questions/:qid')
    async updateAQuestion(@Param('qid') qid: string, @Body() body: any, @Request() req: any) {
        const workspaceId = BigInt(req.user.workspace_id || 1);
        const updaterId = BigInt(req.user.sub || 1);
        return this.service.updateAQuestion(workspaceId, BigInt(qid), updaterId, body);
    }

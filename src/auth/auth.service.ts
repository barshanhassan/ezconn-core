...
export class AuthService {
    ...
    async makeTFA(userId: bigint) {
        ...
        const otpauth = await generateURI({ label: user.email, issuer: 'Ezconn Platform', secret });
    ...
    async logout(userId: bigint) {
        ...
        // we log the event.
        console.log(`User ${userId} logged out (JWT token dropped by client or blacklisted)`);
        return { user: null };
    }
    ...
    async validateInvitation(invitationId: string) {
        ...
        if (!user) {
            throw new BadRequestException('Invalid invitation code or already accepted');
        }
        ...
        return {
            error: false,
            member: { id: user.id.toString(), email: user.email, first_name: user.first_name },
            first_term: firstTerm,
            second_term: secondTerm
        };
    }
    ...
    async verifyEmail(userId: bigint, email: string, code?: string, domainInfo?: any) {
        const user = await this.prisma.users.findUnique({ where: { id: userId } });
        if (!code) {
            // Sending verification code
            const emailExists = await this.prisma.users.findFirst({
                where: { email, modelable_id: domainInfo.modelable_id, modelable_type: domainInfo.modelable_type }
            });
            ...
            console.log(`Stub: Dispatched Email Code ${verificationCode} to ${email}`);
            ...
            if (isVerified) {
                await this.prisma.users.update({ where: { id: userId }, data: { email } });
            }
            return { verified: isVerified };
        }
    }
    ...
    async forgotPassword(email: string, domainInfo: any) {
        ...
        */
        ...
        return { message: 'Password reset email sent', code: 'EMAIL_SENT', debugCode: code };
    }
